...
void Player::Shoot()
{
	...
	// 入力している場合、タイマーを加算する
	if (keyboard.IsPressed(KEY_INPUT_SPACE))
	{
	...
	// 撃つ方向を入力
	auto shootDir = Vector2::Zero;
	if (keyboard.IsPressed(KEY_INPUT_UP))		{ shootDir.y -= 1.0f; }
	if (keyboard.IsPressed(KEY_INPUT_DOWN))		{ shootDir.y += 1.0f; }
	if (keyboard.IsPressed(KEY_INPUT_LEFT))		{ shootDir.x -= 1.0f; }
	if (keyboard.IsPressed(KEY_INPUT_RIGHT))	{ shootDir.x += 1.0f; }
	shootDir.Normalize();

	// 撃つ方向が0でない場合のみ、代入する
	// MEMO : これをしないと撃つ方向が0になり弾がその場に留まってしまう
	if (shootDir != Vector2::Zero)
	{
		_shootDir = shootDir;
	}
	
	// 弾の発射処理 (発射できる弾を探して1発撃つ)
	for (size_t i = 0; i < _bullets.size(); ++i)
	{
		...
		// 弾を撃つ
		_bullets.at(i).OnShoot(_pos, _shootDir, kShootSpeed);
	...
	for (auto& bullet : _bullets)
	{
		...
		bullet.OnShoot(_pos, _shootDir, kShootSpeed);
		break;		
	}
	*/
}
